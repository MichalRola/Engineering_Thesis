...
def audio_to_spectrogram(load, save, file_name, chunk_size, mel):
    counter = 0
    intervals = ffmpeg_way.detect_silence(os.path.join(load, file_name), 1)

    if intervals is None:
        create_melspectrogram(load, save, file_name, counter, 0, 0, chunk_size, mel)
    else:
        for i in range(len(intervals)-1):
            sound_start = int(intervals[i][1]*SAMPLING_RATE)
            sound_end = int(intervals[i+1][0]*SAMPLING_RATE)
            counter = create_melspectrogram(load, save, file_name, counter, sound_start, sound_end, chunk_size, mel)
    del intervals
    gc.collect()
    return
